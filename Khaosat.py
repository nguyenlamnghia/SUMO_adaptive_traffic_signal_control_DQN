...
for i in range(EPOCHS):
    ...
    while step < STEP_SIMULATION:
        ...
        if current_phase == 0:
            reward = throughput/(green_time + YELLOW_TIME + RED_TIME)
            phase_0_reward.append(reward)
            phase_0_actions.append(action)
        elif current_phase == 3:
            reward = throughput/(green_time + YELLOW_TIME + RED_TIME)
            phase_3_reward.append(reward)
            phase_3_actions.append(action)
        else:
            reward = throughput/(green_time + YELLOW_TIME + RED_TIME)
            phase_6_reward.append(reward)
            phase_6_actions.append(action)
    ...
    with open(f"Khaosat_epoch_{i + 1}_config.txt", "w") as f:
        f.write(f"Epoch: {i + 1}\n")
        ...
        f.write(f"phase 0 reward: {np.mean(phase_0_reward):.2f}\n")
        f.write(f"phase 3 reward: {np.mean(phase_3_reward):.2f}\n")
        f.write(f"phase 6 reward: {np.mean(phase_6_reward):.2f}\n")

        f.write(f"phase 0 reward max: {np.max(phase_0_reward):.2f}\n")
        f.write(f"phase 3 reward max: {np.max(phase_3_reward):.2f}\n")
        f.write(f"phase 6 reward max: {np.max(phase_6_reward):.2f}\n")

        f.write(f"Number of action phase 0 max: {phase_0_actions[np.argmax(phase_0_reward)]}\n")
        f.write(f"Number of action phase 3 max: {phase_3_actions[np.argmax(phase_3_reward)]}\n")
        f.write(f"Number of action phase 6 max: {phase_6_actions[np.argmax(phase_6_reward)]}\n")

        f.write(f"phase 0 reward min: {np.min(phase_0_reward):.2f}\n")
        f.write(f"phase 3 reward min: {np.min(phase_3_reward):.2f}\n")
        f.write(f"phase 6 reward min: {np.min(phase_6_reward):.2f}\n")

        f.write(f"Number of action phase 0 min: {phase_0_actions[np.argmin(phase_0_reward)]}\n")
        f.write(f"Number of action phase 3 min: {phase_3_actions[np.argmin(phase_3_reward)]}\n")
        f.write(f"Number of action phase 6 min: {phase_6_actions[np.argmin(phase_6_reward)]}\n")
