...
EPOCHS = 6  # Tăng số epoch để minh họa
STEP_LENGTH = 0.1
...
# Hàm tính throughput trong suốt chu kỳ
def get_throughput(junction_id, steps):
    """
    Tính số xe rời khỏi nút giao trong suốt một chu kỳ (gồm nhiều bước mô phỏng).
    junction_id: ID của nút giao (e.g., "J3").
    steps: Số bước mô phỏng trong chu kỳ (green_time + yellow_time + red_time).
    Trả về: Số xe đã rời khỏi nút giao.
    """
    lanes = get_total_vehicle_in_junction(junction_id)
    count = 0
    
    for _ in range(steps):
        # Lấy tất cả xe hiện tại trên các làn đường
        current_vehicles = set(get_total_vehicle_in_junction(junction_id))

        # Tiến hành bước mô phỏng
        traci.simulationStep()
        
        # Các xe đã rời khỏi là những xe trong seen_vehicles nhưng không còn trong current_vehicles
        ...
        for veh_id in throughput:
            if traci.vehicle.getVehicleClass(veh_id) == "passenger":
                ...
                count += 1  # Default case for other vehicle types
        # Cập nhật current_vehicles cho bước tiếp theo
...
for i in range(EPOCHS):
    ...
    while step < STEP_SIMULATION:
        ...
        throughput = apply_action(current_phase, green_time)
        print("THROUGHPUT:", throughput)
        reward = (1+0.05*(action))*(throughput/green_time/3) if current_phase == 0 else (1+0.05*(action))*(throughput/green_time)
