...
def get_state(incoming_vehicles_count):
    ...
    incoming_vehicle_values = [incoming_vehicles_count[d] for d in detectors]
    ...
    return np.array(number_of_vehicle_values + incoming_vehicle_values + [current_phase])
...
for i in range(EPOCHS):
    ...
    while step < STEP_SIMULATION:
        ...
        state = get_state(incoming_vehicles_count)
        ...
        if current_phase == 0:
            reward = throughput/(green_time + YELLOW_TIME + RED_TIME)/2.5
        ...
        new_state = get_state(incoming_vehicles_count)
