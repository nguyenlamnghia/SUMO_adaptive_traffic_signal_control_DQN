...
def run_simulation_for_time(duration_seconds):
    ...
    print(f"Count throughput: {count_throughput}, Vehicles through: {vehicles_through}")
    if current_phase == 0:
        count_throughput = count_throughput/2.48
    elif current_phase == 3:
        count_throughput = count_throughput/0.48
    elif current_phase == 6:
        count_throughput = count_throughput/0.5
    else:
        count_throughput = 0
...
for i in range(EPOCHS):
    ...
    while step < STEP_SIMULATION:
        ...
        if time_in_current_phase >= MIN_GREEN_TIME and time_in_current_phase % DECISION_INTERVAL == 0:
            ...
            if action == 0:  # Keep current phase
                ...
                reward = calculate_reward(action, throughput, DECISION_INTERVAL)
                ...
                reward = calculate_reward(action, throughput, 19)

                time_in_current_phase = 10  # Already spent 10s in new phase
                step += 19 * SEC_TO_STEP
                
                print(f"Switching from phase {current_phase} to next phase")
                print(f"Total time: 19s, Throughput: {throughput}, Reward: {reward:.4f}")
            ...
            if len(vehicles_in_junction) > 50:
    ...
    with open(f"epoch_{i + 1}_config_main_2.txt", "w") as f:
        f.write(f"Epoch: {i + 1}\n")
    ...
    print("Saving model...")
    dqn.save("quad_dqn_main_2.keras")
