...
def get_state(incoming_vehicles_count, action=None):
    ...
    state = np.array( number_of_vehicle_values + average_flow+ [current_phase])
    print(f"STATE: {state}")
    return state
...
try:
    dqn = QuadDQN.load("quad_dqn_2_3.keras")
...
for i in range(EPOCHS):
    ...
    while step < STEP_SIMULATION:
        ...
        if current_phase == 0:
            reward = throughput/(green_time + YELLOW_TIME + RED_TIME)/2.69
        elif current_phase == 3:
            reward = throughput/(green_time + YELLOW_TIME + RED_TIME)/0.71
        else:
    ...
    with open(f"epoch_{i + 1}_config_2_3.txt", "w") as f:
        f.write(f"Epoch: {i + 1}\n")
