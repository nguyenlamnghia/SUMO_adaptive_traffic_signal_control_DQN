...
def get_state(incoming_vehicles_count, action=None):
    ...
    state = np.array( number_of_vehicle_values + mean_speech_values + [current_phase])
    print(f"STATE: {state}")
    return state
...
for i in range(EPOCHS):
    ...
    while step < STEP_SIMULATION:
        ...
        if current_phase == 0:
            reward = throughput/(green_time + YELLOW_TIME + RED_TIME)/2.55
        elif current_phase == 3:
            reward = throughput/(green_time + YELLOW_TIME + RED_TIME)/0.78
        else:
            reward = throughput/(green_time + YELLOW_TIME + RED_TIME)/0.43
    ...
    with open(f"epoch_{i + 1}_config_update_state.txt", "w") as f:
        f.write(f"Epoch: {i + 1}\n")
    ...
    dqn.save("quad_dqn_update_state.keras")
