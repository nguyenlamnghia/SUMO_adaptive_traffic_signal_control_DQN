...
class TrafficLightEnv:
    ...
    def get_metrics(self):
        ...
        total_waiting_time = sum(traci.lane.getWaitingTime(d) for d in DETECTORS)
        total_vehicles = sum(traci.lane.getLastStepVehicleNumber(d) for d in DETECTORS)
        avg_waiting_time = total_waiting_time / total_vehicles if total_vehicles > 0 else 0
        queue_length = total_vehicles
...
def train_dqn(epochs):
    ...
    for i in range(epochs):
        ...
        while env.step < STEP_SIMULATION:
            current_waiting_time = env._get_waiting_time()
            state = env.get_state()
